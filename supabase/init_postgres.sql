...
-- === Hybrid Search (Scheme A): FTS (tsvector) + pgvector (dense embeddings) ===
-- Ensure extensions exist
create extension if not exists vector with schema extensions;   -- pgvector for dense search
create extension if not exists pg_trgm with schema extensions;  -- optional, for fuzzy matching/ILIKE acceleration

-- 1) Full-text search on short text fields (no large bodies stored in DB)
-- We use course_name (A), exam_form (B), workload (C). You can add more short fields later.
alter table public.courses
  add column if not exists fts tsvector generated always as (
    setweight(to_tsvector('simple', coalesce(course_name,'')), 'A') ||
    setweight(to_tsvector('simple', coalesce(exam_form,'')), 'B') ||
    setweight(to_tsvector('simple', coalesce(workload,'')), 'C')
  ) stored;

create index if not exists courses_fts_idx on public.courses using gin(fts);

-- Optional: speed up fuzzy matching on course_name
create index if not exists courses_name_trgm on public.courses using gin (course_name gin_trgm_ops);

-- 2) Dense embeddings table (course-level). Long texts stay in object storage; we only keep a short preview + URL.
create table if not exists public.course_embeddings (
  ...
  embedding vector(1024) not null,             -- bge-m3 dense size (adjust if you use other dims)
  preview text,                                -- short snippet for UI
  source_url text                              -- object storage path or external URL
);

-- Vector index: HNSW (Postgres 16+) or IVFFlat (Postgres 15)
-- Use cosine similarity for BGE models
create index if not exists course_embeddings_hnsw on public.course_embeddings using hnsw (embedding vector_cosine_ops);
-- If you're on PG15, comment the HNSW index above and use:
-- create index if not exists course_embeddings_ivf on public.course_embeddings using ivfflat (embedding vector_cosine_ops) with (lists = 100);

-- 3) RPC: Hybrid search that fuses FTS rank and dense cosine score
create or replace function public.hybrid_search_courses(
  q text,
  query_embedding vector,
  k int default 30,
  w_fts float4 default 0.6,
  w_dense float4 default 0.4
)
returns table(
  id bigint,
  course_name text,
...
language sql stable as $$
  with fts as (
    select c.id, ts_rank(c.fts, plainto_tsquery('simple', q)) as fts_rank
    from public.courses c
    where c.fts @@ plainto_tsquery('simple', q)
  ),
  dense as (
    select ce.course_id as id, 1 - (ce.embedding <=> query_embedding) as dense_score
    from public.course_embeddings ce
    order by ce.embedding <=> query_embedding
    limit greatest(k, 100)
  ),
  joined as (
  ...
  select id, course_name,
         (w_fts * fts_rank) + (w_dense * dense_score) as score
  from joined
  where fts_rank > 0 or dense_score > 0
  order by score desc
  limit k;
$$;
...
DO $$ BEGIN
  ...
  IF FOUND THEN
    GRANT EXECUTE ON FUNCTION public.hybrid_search_courses(text, vector, int, float4, float4) TO anon;
    GRANT SELECT   ON public.course_embeddings TO anon;
  END IF;
  PERFORM 1 FROM pg_roles WHERE rolname = 'authenticated';
  IF FOUND THEN
    GRANT EXECUTE ON FUNCTION public.hybrid_search_courses(text, vector, int, float4, float4) TO authenticated;
    GRANT SELECT   ON public.course_embeddings TO authenticated;
  END IF;
END $$;

-- RLS for course_embeddings: read-only
alter table public.course_embeddings enable row level security;
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='course_embeddings' AND policyname='read_course_embeddings'
  ) THEN
    CREATE POLICY read_course_embeddings ON public.course_embeddings FOR SELECT
...
values ('MA', NULL, 'MA Project Autumn'), ('MA', NULL, 'MA Project Spring')
...
semester_agg as (
  select
    course_id,
    case
      when bool_or(inferred_semester = 'winter') then 'winter'
      when bool_or(inferred_semester = 'summer') then 'summer'
      else null
    end as preferred_semester
  from normalized_programs
  group by course_id
)
select
  ...
  c.credits,
  c.lang,
  coalesce(semester_agg.preferred_semester, c.semester) as semester,
  c.exam_form,
  c.workload,
