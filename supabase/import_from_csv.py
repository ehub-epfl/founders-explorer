"""
Import EPFL course data from local data files into Supabase Postgres.
...
Requirements:
  ...
  - CSV/Parquet inputs generated by data-scraper:
      data-scraper/data/epfl_courses.csv
      data-scraper/data/courses_scores.parquet
      data-scraper/data/courses_texts.csv
...
SCORES_PARQUET = DATA_DIR / "courses_scores.parquet"
SCORES_CSV_LEGACY = DATA_DIR / "courses_scores.csv"
COURSE_TEXTS_CSV = DATA_DIR / "courses_texts.csv"

ASPECT_TO_SCORE_KEY = {
    "personal_development": "score_skills_sigmoid",
    "product_development": "score_product_sigmoid",
    "venture_building": "score_venture_sigmoid",
    "entrepreneurship_foundations": "score_foundations_sigmoid",
}
...
def create_score_entry() -> Dict[str, Any]:
    return {
        "score_skills_sigmoid": None,
        "score_product_sigmoid": None,
        "score_venture_sigmoid": None,
        "score_foundations_sigmoid": None,
        "embeddings": None,
        "preview": None,
    }


def load_scores_csv(path: Path) -> Dict[str, Dict[str, Any]]:
        ...
        for row in reader:
            ...
            if embeddings_raw:
                try:
                    ...
                    embeddings_vec = None
            entry["score_skills_sigmoid"] = parse_float(row.get("score_skills_sigmoid"))
            entry["score_product_sigmoid"] = parse_float(row.get("score_product_sigmoid"))
            entry["score_venture_sigmoid"] = parse_float(row.get("score_venture_sigmoid"))
            entry["score_foundations_sigmoid"] = parse_float(row.get("score_foundations_sigmoid"))
            entry["embeddings"] = embeddings_vec
            preview_text = (row.get("text") or "").strip()
            entry["preview"] = preview_text or None
            scores[row_id] = entry
    return scores


def load_scores_parquet(path: Path) -> Dict[str, Dict[str, Any]]:
    try:
        import pandas as pd  # type: ignore
    except ImportError as exc:  # pragma: no cover - optional dependency
        raise RuntimeError(
            "Reading Parquet scores requires pandas (with pyarrow or fastparquet). "
            "Install the dependency or provide courses_scores.csv instead."
        ) from exc

    df = pd.read_parquet(path)
    scores: Dict[str, Dict[str, Any]] = {}
    for row in df.to_dict("records"):
        row_id_raw = row.get("course_id") or row.get("row_id")
        row_id = str(row_id_raw).strip() if row_id_raw is not None else ""
        if not row_id:
            continue
        entry = scores.get(row_id)
        if entry is None:
            entry = create_score_entry()
            scores[row_id] = entry
        aspect_raw = row.get("aspect")
        aspect = str(aspect_raw).strip().lower() if aspect_raw is not None else ""
        score_key = ASPECT_TO_SCORE_KEY.get(aspect)
        if score_key:
...
def load_course_texts(path: Path) -> Dict[str, Dict[str, Any]]:
    if not path.exists():
        return {}
    ...
    with path.open(newline="", encoding="utf-8") as handle:
        ...
        for row in reader:
            ...
            if not row_id:
                continue
            embeddings_vec = None
            embeddings_raw = row.get("embedding")
            if embeddings_raw:
                try:
                    parsed = json.loads(embeddings_raw)
                    if isinstance(parsed, list):
                        ...
                        for item in parsed:
                            try:
                                embeddings_vec.append(float(item))
                            except (TypeError, ValueError):
                                embeddings_vec = None
                                break
                        if embeddings_vec is not None and not embeddings_vec:
                            embeddings_vec = None
                except json.JSONDecodeError:
                    embeddings_vec = None
            preview_text = (row.get("text") or "").strip()
...
def load_scores(path: Path) -> Dict[str, Dict[str, Any]]:
    ...
    if suffix == ".parquet":
        return load_scores_parquet(path)
    if suffix == ".csv":
        return load_scores_csv(path)
...
def build_payloads(
    courses_csv: Path, scores_path: Path
) -> Tuple[
    ...
    if not scores_path.exists():
        legacy_path = SCORES_CSV_LEGACY
        if legacy_path.exists():
            scores_path = legacy_path
        else:
            raise FileNotFoundError(f"Missing score file {scores_path}")

    scores_map = load_scores(scores_path)

    course_texts = load_course_texts(COURSE_TEXTS_CSV)
    for row_id, payload in course_texts.items():
        entry = scores_map.get(row_id)
        if entry is None:
            entry = create_score_entry()
            scores_map[row_id] = entry
        embedding_vec = payload.get("embedding")
        if embedding_vec is not None:
            entry["embeddings"] = embedding_vec
        ...
        for row in reader:
            ...
            if row_id:
                ...
                if emb and course_code not in embeddings_map:
                    preview_raw = score.get("preview")
                    preview = preview_raw.strip() if isinstance(preview_raw, str) else ""
...
def main() -> None:
    ...
    (
        ...
        embeddings_map,
        unparsed_program_labels,
    ) = build_payloads(COURSES_CSV, SCORES_PARQUET)
