"""
Import EPFL course data from locally generated files into Supabase Postgres.
...
Requirements:
  ...
  - Data inputs generated by data-scraper:
...
ASPECT_FIELD_MAP = {
    "entrepreneurship_relevance": "score_relevance_sigmoid",
...
def load_scores(path: Path) -> Dict[str, Dict[str, Any]]:
    try:
        ...
        raise RuntimeError(
            "pyarrow is required to read courses_scores.parquet. "
    ...
    required_cols = {"course_id", "aspect", "score"}
    missing = required_cols.difference(table.column_names)
    if missing:
        missing_cols = ", ".join(sorted(missing))
        raise ValueError(f"courses_scores.parquet is missing columns: {missing_cols}")

    records = table.to_pylist()
    scores: Dict[str, Dict[str, Any]] = {}
    for record in records:
        course_id = record.get("course_id")
        row_id = str(course_id).strip() if course_id is not None else ""
        if not row_id:
            continue
        aspect = record.get("aspect")
        field_name = ASPECT_FIELD_MAP.get(aspect)
        if not field_name:
            continue
        entry = scores.setdefault(
            row_id,
            {
                "score_relevance_sigmoid": None,
        ...
        entry[field_name] = parse_float(record.get("score"))
    return scores
...
def build_payloads(
    ...
    if not scores_path.exists():
        raise FileNotFoundError(f"Missing Parquet {scores_path}")
        ...
        for row in reader:
            ...
            if row_id:
                ...
                offerings.append({
                    ...
                    "score_relevance_sigmoid": score.get("score_relevance_sigmoid"),
...
def main() -> None:
    ...
    for item in offerings:
        ...
        offering_rows.append({
            ...
            "score_relevance_sigmoid": item["score_relevance_sigmoid"],
